...
export default function ReservarCanchaUsuario({ user }) {
  ...
  const canchasFiltradas = canchas.filter(c => {
    ...
    const tipoOk = !filtroTipo || c.tipoEspacio_id === Number(filtroTipo);
    return nombreOk && tipoOk;
  });

  const tieneAgendada = misReservas.some(r => r.estado === 'agendado');
  ...
  return (
    <div>
      ...
      {tieneAgendada && (
        <div className="alert alert-info mb-3">Solo puedes tener una reserva en estado <b>agendado</b> a la vez.</div>
      )}
